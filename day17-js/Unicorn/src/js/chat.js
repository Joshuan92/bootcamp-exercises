...
const chat = (talk) => {
  for (let i = 0; i < conversation.length; i++) {
    if (talk[i].side === 'local') {
      const chatBox = document.querySelector('#chat');
      const reply = (`<div class="local">
          <span class="face"><img src="/img/cleverbot.png" alt="Cleverbot"></span>
          <span class="name">${talk[i].name}</span>
          <span class="text">${talk[i].text}</span>
         </div>`);

      chatBox.innerHTML += reply;
    } else {
      const chatBox = document.querySelector('#chat');
      const reply = (`<div class="remote">
          <span class="face"><img src="/img/unicorn.png" alt="Unicorn"></span>
          <span class="name">${talk[i].name}</span>
          <span class="text">${talk[i].text}</span>
         </div>`);
...
document.addEventListener('DOMContentLoaded', chat(conversation));
